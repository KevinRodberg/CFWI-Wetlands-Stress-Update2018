...
for (dbk in unique.stations[unique.stations != 'Date']){
  ...
  AllStations_SJR <- rbind(AllStations_SJR,OneStation)
}
names(AllStations_SJR)[names(AllStations_SJR) == 'dbk'] <- 'STATION'
start = 2011
end = 2017
drange = 2011
for (drange in seq(start,end)){
  ich = paste0('2006-',drange)
  AllStations_SJR[format.Date(AllStations_SJR$DATE, "%Y") <= as.character(drange),ich] <-as.double(drange)
}
...
for (yr in seq(start,end)){
  ich = paste0('2006-',yr)
  qStations <- AllStations_SJR[!is.na(AllStations_SJR$approx) & 
                             AllStations_SJR$DATE <= as.Date(paste0(yr,'-12-31')),]
...
PrankFile = paste0(workdir,'SJRWMD_Pranks.csv')
csvStatus %<-% write.csv(Pranks,PrankFile, row.names=FALSE)
DataTable = paste0(workdir,'SJRWMD_DataTable.csv')
csvStatus %<-% write.csv(AllStations_SJR[,1:4],DataTable, row.names=FALSE)

p80<-dcast(Pranks[Pranks$prank=='P80',],STATION~DateRange+prank,mean )
PrankFile = paste0(workdir,'../SJRWMD_P80.csv')
...
for (stn in unique.stations){
  ...
  OneStation <- AllStations_SJR[AllStations_SJR$STATION ==stn,]
