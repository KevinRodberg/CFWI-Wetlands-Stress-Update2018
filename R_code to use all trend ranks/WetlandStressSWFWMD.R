...
for (drange in seq(start,end)){
  ...
  AllStations_SW[format.Date(AllStations_SW$DATE, "%Y") <= as.character(drange),ich] <-as.double(drange)
}
...
for (yr in seq(start,end)){
  #  ich = paste0('2006-',yr)
  ich = paste0(yr,'-2017')
  qStations <- AllStations_SW[!is.na(AllStations_SW$approx) & AllStations_SW$DATE <= as.Date(paste0(yr,'-12-31')),]
