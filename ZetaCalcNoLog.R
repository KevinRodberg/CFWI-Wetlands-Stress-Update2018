...
for (phys in physVec) {
  ...
  if (phys == "Plain") {
   thetas[thetas$phys ==phys & thetas$Stress == 'Not Stressed',]$theta.logN <-
     log(thetas[thetas$phys == phys & thetas$Stress == 'Not Stressed',]$theta+10)
...
for (phys in physVec) {
  ...
  #  SD <- 1.75
    ...
    cat('\n')
 #   Wetlands[Wetlands$phys == phys,]$Ps <- dnorm(log(Wetlands[Wetlands$phys == phys,]$theta+15), Mean, SD)
  ...
  #  SD <- 0.95
    ...
    Wetlands[Wetlands$phys == phys,]$Pu <- dnorm(log(Wetlands[Wetlands$phys == phys,]$theta+15), Mean, SD)
