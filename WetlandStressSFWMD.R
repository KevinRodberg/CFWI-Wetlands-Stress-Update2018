...
AllStations <- merge(stations.SFWMD[,c("STATION","DBKEY","Height")], 
                     AllStations[AllStations$DATE >= '2006-01-01' 
                                 & AllStations$DATE < '2018-01-01', ], 
                     by ="DBKEY",by.y="dbk")
AllStations$approx <- AllStations$approx + AllStations$Height
AllStations$Height <- NULL
start = 2011
end = 2017
...
PrankFile = paste0(workdir,'SFWMD_Pranks.csv')
csvStatus %<-% write.csv(Pranks,PrankFile, row.names=FALSE)

DataTable = paste0(workdir,'SFWMD_DataTable.csv')
AllStations <- AllStations[order(AllStations$STATION,AllStations$DATE),]
csvStatus %<-% write.csv(AllStations[,-c(1)],DataTable, row.names=FALSE)

p80<-dcast(Pranks[Pranks$prank=='P80',],STATION~DateRange+prank,mean )
PrankFile = paste0(workdir,'../SFWMD_P80.csv')
